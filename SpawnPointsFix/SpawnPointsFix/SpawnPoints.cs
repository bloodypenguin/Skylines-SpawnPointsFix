    ...
    public class SpawnPoints
    {
        ...
        public static void RecalculateSpawnPoints(BuildingInfo info)
        {
            ...
            var allSpawnPoints = (from path in paths
                ...
                select new KeyValuePair<Vector3, ItemClass>(GetMiddle(path), path.m_netInfo.m_class)).Distinct().ToArray();
            ...
            else
            {
                primarySpawnPoints = allSpawnPoints.
                    Where(kvp => kvp.Value.m_subService == buildingAI.m_transportInfo.m_class.m_subService || kvp.Value.m_subService == ItemClass.SubService.None && kvp.Value.m_service == ItemClass.Service.Road && buildingAI.m_transportInfo.m_class.m_subService == ItemClass.SubService.PublicTransportBus).
                    Select(kvp => kvp.Key).
                    ToArray(); 
                secondarySpawnPoints = allSpawnPoints.
                    Where(kvp => kvp.Value.m_subService == buildingAI.m_secondaryTransportInfo.m_class.m_subService || kvp.Value.m_subService == ItemClass.SubService.None && kvp.Value.m_service == ItemClass.Service.Road && buildingAI.m_secondaryTransportInfo.m_class.m_subService == ItemClass.SubService.PublicTransportBus).
                    Select(kvp => kvp.Key).
                    ToArray();
            }
